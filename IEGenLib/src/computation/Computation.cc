...
std::string Computation::codeGen(Set* knownConstraints) {
    ...
    for (Relation* rel : transformations) {
        ...
        Set* tempSpace = new Set(*iterSpace);
       	tempSpace->pushConstToConstraints();
        stmtMacroDefs << "#define s_" << stmtCount << "("
                      << tempSpace->getTupleDecl().toString() << ")   "
	...
	delete tempSpace;
        ...
        tempSpace = new Set(*newIterSpace);
        tempSpace->pushConstToConstraints();	
	stmtMacroDefs << "#define s" << stmtCount << "("
                      << tempSpace->getTupleDecl().toString()
                      << ")   s_" << stmtCount << "(";
        delete tempSpace;
        // tempspoace is used to prevent the macros from having 
	// constants in its lhs
	...
	// Generate mapping from out tuple -> in tuple
        ...
        for (int i = 0; i < rel->inArity(); i++) {
            ...
            stmtMacroDefs << (e && !e->equalsZero() ?
                             e->prettyPrintString(decl) : "__x"+std::to_string(i));
            if (e) {
                delete e;
            }
